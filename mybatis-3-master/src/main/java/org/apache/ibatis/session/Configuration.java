...
public class Configuration {
  ...
  /*返回空行实例--null行不返回数据，就是行都是null值的，可能比如一些报表如果都返会null，那么可能需要配置这个值，有可能有时候返回 0 也是不映射的，在解析构造row数据的时候不会返回这个对象 ，而是返会null */
  ...
  /*数据库id 用于匹配statement，不同数据库的statement（sql语句）可能是不一样的*/
  ...
  /*存放所有已经解析好的 MappedStatement （也就是sql关系）*/
  ...
  /*用于判断接口Mapper资源是否已经被加载*/
  protected final Set<String> loadedResources = new HashSet<>();
  /*sql片段*/
  protected final Map<String, XNode> sqlFragments = new StrictMap<>("XML fragments parsed from previous mappers");

  /*记录如下这些未完成的statements，然后进行一次重试*/
  ...
  public ParameterHandler newParameterHandler(MappedStatement mappedStatement, Object parameterObject, BoundSql boundSql) {
    ...
    /*通过使用JDK代理ParameterHandler接口 在执行 ParameterHandler 的方法的时候实现增强操作*/
  ...
  public ResultSetHandler newResultSetHandler(Executor executor, MappedStatement mappedStatement, RowBounds rowBounds, ParameterHandler parameterHandler,
    ...
    /*通过使用JDK代理ResultSetHandler接口 在执行 ResultSetHandler 的方法的时候实现增强操作*/
  ...
  public Executor newExecutor(Transaction transaction, ExecutorType executorType) {
    ...
    /*默认是开启这个缓存，使用的是装饰器模式，增强方法添加缓存 ，但是具体用不用还要看配置有没有cache，缓存（这个是二级缓存） */
    if (cacheEnabled) {
